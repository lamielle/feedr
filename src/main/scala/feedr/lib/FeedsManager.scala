...
import actor._
import common.{Empty, Box, Full, Logger}
import feedr.model.Feed
import feedr.comet.FeedEditor.ProvideFeed

// Singleton actor responsible for creating new feeds
object FeedsManager extends LiftActor with Logger {
  case class NewFeed()
  case class RequestFeed(feedId: String, actor: LiftActor)

  override def messageHandler = {
    case NewFeed() => reply(newFeed)
    case RequestFeed(feedId, actor: LiftActor) => {
      val feedOption = feeds get feedId
      val feedBox: Box[Feed] = feedOption.map(Full(_)) openOr Empty
      actor ! ProvideFeed(feedBox)
    }
